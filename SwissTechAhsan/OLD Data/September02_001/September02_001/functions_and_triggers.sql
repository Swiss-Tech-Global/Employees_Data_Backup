...
DECLARE
    ...
    party_acc BIGINT;
BEGIN
	-- DELETE old journal if needed
    IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
        DELETE FROM JournalEntries WHERE journal_id = OLD.journal_id;
    END IF;

    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        -- Use AP account for vendor or Both
        SELECT ap_account_id INTO party_acc FROM Parties WHERE party_id = NEW.party_id;

        INSERT INTO JournalEntries(entry_date, description)
        VALUES (NEW.payment_date, 'Payment to ' || NEW.party_id)
        RETURNING journal_id INTO j_id;

		-- Prevent recursion when updating journal_id
		PERFORM pg_catalog.set_config('session_replication_role', 'replica', true);
        UPDATE Payments SET journal_id = j_id WHERE payment_id = NEW.payment_id;
...
BEGIN
    ...
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        -- Use AR account for customer or Both
        SELECT ar_account_id INTO party_acc FROM Parties WHERE party_id = NEW.party_id;
        ...
        PERFORM pg_catalog.set_config('session_replication_role', 'replica', true);
        UPDATE Receipts SET journal_id = j_id WHERE receipt_id = NEW.receipt_id;
		PERFORM pg_catalog.set_config('session_replication_role', 'origin', true);
